 ...
 */
package com.xiezongnan.gen.codegen.entity;
...
public class Table {
     ...
     * 构建 dto&vo import 导包。
     */
    public List<String> buildDtoAndVoImports() {
        Set<String> imports = new HashSet<>();
        for (Column column : columns) {
            imports.addAll(column.buildDtoAndVoImports());
        }

        DtoConfig dtoConfig = globalConfig.getDtoConfig();

        if (dtoConfig.getSuperClass() != null) {
            imports.add(dtoConfig.getSuperClass().getName());
        }

        if (dtoConfig.getImplInterfaces() != null) {
            for (Class<?> entityInterface : dtoConfig.getImplInterfaces()) {
                imports.add(entityInterface.getName());
            }
        }
    ...
    /**
     * @see templates/enjoy/dto.tpl
     * @return
     */
    public String buildDtoClassName() {
        String entityJavaFileName = getEntityJavaFileName();
        DtoConfig dtoConfig = globalConfig.getDtoConfig();
        return dtoConfig.getClassPrefix()
            + entityJavaFileName
            + dtoConfig.getClassSuffix();
    }

    /**
     * @see templates/enjoy/vo.tpl
     * @return
     */
    ...
    /**
     * 构建 mapstruct 的 Class 名称。
     */
    public String buildMapstructClassName() {
        String entityJavaFileName = getEntityJavaFileName();
        MapstructConfig mapstructConfig = globalConfig.getMapstructConfig();
        return mapstructConfig.getClassPrefix()
            + entityJavaFileName
            + mapstructConfig.getClassSuffix();
    }
}
